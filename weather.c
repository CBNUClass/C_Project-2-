...
printf("  %2d. %s (X: %s, Y: %s)\n", i + 1, locations[i].name, locations[i].nx, 
locations[i].ny); 
} 
int choice; 
printf("번호 입력: "); 
...
"fHhnNwA7fGBGdq%2FTX99FNNLQJh6pa3CQTHUPpKpk%2FyNHVqEzIDueYm2EKXOq7%2
 BfjY4fS4KpjCEQBoG3oQ0tTaQ%3D%3D"; 
char url[1024]; 
snprintf(url, sizeof(url), 
"http://apis.data.go.kr/1360000/VilageFcstInfoService_2.0/" 
"getUltraSrtNcst?serviceKey=%s&pageNo=1&numOfRows=100&dataType=JSON" 
"&base_date=%s&base_time=%s&nx=%s&ny=%s", 
serviceKey, base_date, base_time, locations[choice - 1].nx, locations[choice - 
1].ny); 
        ...
        if (res != CURLE_OK) { 
            ...
            if (root) { 
                ...
                for (int i = 0; i < arraySize; ++i) { 
                    ...
                    const char* category = cJSON_GetObjectItem(item, "category")
>valuestring; 
                    const char* obsrValue = cJSON_GetObjectItem(item, "obsrValue")
>valuestring; 
